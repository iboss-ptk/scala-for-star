...
//this examples are from 'https://github.com/rubbish/scala-koans/tree/master/src/test/scala/org/scalakoans'
...
class List$Test  extends FunSuite with Matchers {
  ...
  test ("Lists have many useful methods") {
    ...
    a.map { v => ???} should equal (List(2,6,10,14,18))

    // filter out any values divisible by 3 in the list
    a.filter { v => ??? } should equal (List(3,9))
  }
